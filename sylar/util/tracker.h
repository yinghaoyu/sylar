...
class Tracker : public std::enable_shared_from_this<Tracker> {
 ...
 public:
  ...
  typedef std::function<void(const std::vector<std::string>& dims,
                             int64_t* datas, int64_t interval)>
      callback;
  ...
  void toData(std::map<std::string, int64_t>& data,
              const std::set<uint32_t>& times = {});
  void toDurationData(std::map<std::string, int64_t>& data,
                      uint32_t duration = 1, bool with_time = true);
  void toTimeData(std::map<std::string, int64_t>& data, uint32_t ts = 1,
                  bool with_time = true);
