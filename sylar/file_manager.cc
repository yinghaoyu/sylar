...
std::string FileInfo::toString() const {
  std::stringstream ss;
  ss << "[FileInfo path=" << m_path << " md5=" << m_md5
     << " data_size=" << m_size << " mtime=" << sylar::Time2Str(m_mtime)
...
FileInfo::ptr FileInfoManager::load(const std::string& path, uint64_t limit) {
  ...
  do {
    ...
    if (stat(path.c_str(), &st) != 0) {
      SYLAR_LOG_ERROR(g_logger)
          << "stat(" << path << ") error:" << strerror(errno);
      return nullptr;
    }
  ...
  SYLAR_LOG_INFO(g_logger) << "load " << path << " speed=" << (int64_t)limit
                           << " fail. used: "
                           << (sylar::GetCurrentUS() - ts) / 1000.0 << "ms"
                           << " " << strerror(errno);
  return nullptr;
}

bool FileInfoManager::load(const std::string& path, callback cb, uint64_t limit,
                           bool auto_clear) {
