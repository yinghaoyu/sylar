...
std::string GrpcResponse::toString() const {
  std::stringstream ss;
  ss << "[GrpcResponse result=" << m_result << " error=" << m_error
     << " response=" << m_response << "]";
  return ss.str();
}

GrpcResponse::ptr GrpcConnection::request(GrpcRequest::ptr req,
                                          uint64_t timeout_ms) {
...
GrpcResponse::ptr GrpcConnection::request(
  ...
  http::HttpRequest::ptr http_req =
      std::make_shared<http::HttpRequest>(0x20, false);
  ...
  if (!message.SerializeToString(&msg->data)) {
    SYLAR_LOG_ERROR(g_logger)
        << "SerializeToString fail, " << sylar::PBToJsonString(message);
    GrpcResponse::ptr rsp = std::make_shared<GrpcResponse>();
    rsp->setResult(-100);
