...
void Tracker::start() {
  ...
  if (iom) {
    m_timer = iom->addTimer(
        100, std::bind(&Tracker::onTimer, shared_from_this()), true);
  } else {
...
void Tracker::toData(std::map<std::string, int64_t>& data,
                     const std::set<uint32_t>& times) {
  time_t now = time(0) - 1;
...
void Tracker::toDurationData(std::map<std::string, int64_t>& data,
                             uint32_t duration, bool with_time) {
  time_t now = time(0) - 1;
...
void Tracker::toTimeData(std::map<std::string, int64_t>& data, uint32_t ts,
                         bool with_time) {
  if (ts >= m_interval) {
    return;
  }
...
void Tracker::addDim(uint32_t key, const std::string& name) {
  if (m_timer) {
    SYLAR_LOG_ERROR(g_logger)
        << "Tracker is running, addDim key=" << key << " name=" << name;
    return;
  }
  if (key >= m_dims.size()) {
    m_dims.resize(key + 1);
...
std::string TrackerManager::toString(uint32_t duration) {
  ...
  if (m_totalTracker) {
    ...
    for (auto& n : data) {
      ss << std::setw(40) << std::right << n.first << ": " << n.second
         << std::endl;
    }
  }
  for (auto& i : datas) {
