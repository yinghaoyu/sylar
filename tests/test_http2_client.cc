...
void test() {
  ...
  // sylar::Address::LookupAnyIPAddress("192.168.1.6:8099");
  sylar::Address::ptr addr =
      sylar::Address::LookupAnyIPAddress("127.0.0.1:8099");
  sylar::http2::Http2Connection::ptr stream(
      new sylar::http2::Http2Connection());
  // if(!stream->connect(addr, true)) {
  if (!stream->connect(addr, false)) {
    std::cout << "connect " << *addr << " fail";
  }
  // if(stream->handleShakeClient()) {
  //     std::cout << "handleShakeClient ok," << stream->isConnected() <<
  //     std::endl;
  // } else {
