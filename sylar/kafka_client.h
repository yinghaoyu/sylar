...
class KafkaProducerGroup {
 ...
 private:
  struct Msg {
    ...
    Msg(const std::string& _topic, const std::string& _msg,
...
class KafkaConsumer {
 public:
  ...
  typedef std::function<void(const RdKafka::Message& msg,
  ...
  bool setBrokerList(const std::string& bl);
  // bool produce(const std::string& topic, const std::string& msg);
  // bool produce(const std::string& topic, const std::string& msg, const
  // std::string& key);
  std::string getErr() const { return m_err; }
