...
void test_batch(sylar::SQLite3::ptr db) {
  ...
  SYLAR_LOG_INFO(g_logger) << "used: " << (ts2 - ts) / 1000.0
                           << "s batch insert n=" << n;
}
...
#undef XX
    ...
    if (rt != SQLITE_OK) {
      SYLAR_LOG_ERROR(g_logger) << "create table error " << db->getErrorCode()
                                << " - " << db->getErrorMsg();
      return 0;
    }
  }
  ...
  if (!stmt) {
    SYLAR_LOG_ERROR(g_logger) << "create statement error " << db->getErrorCode()
                              << " - " << db->getErrorMsg();
    return 0;
  }
  ...
  if (!ds) {
    SYLAR_LOG_ERROR(g_logger)
        << "query error " << db->getErrorCode() << " - " << db->getErrorMsg();
    return 0;
  }
