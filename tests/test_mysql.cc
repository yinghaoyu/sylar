...
void run() {
  do {
    ...
    auto res = std::dynamic_pointer_cast<sylar::MySQLStmtRes>
        ...
        (mysql->queryStmt("select *, 'hello' as xx from abt_pageids_gd9 where "
                          "status >= ? and status <= ?",
                          a, a));
    ...
    if (res->getErrno()) {
      std::cout << "errno=" << res->getErrno() << " errstr=" << res->getErrStr()
                << std::endl;
      return;
    }
    ...
    while (res->next()) {
      ++i;
      std::cout << res->getInt64(0) << " - " << res->getInt64(1) << std::endl;
    }
    std::cout << "===" << i << std::endl;
  } while (false);
